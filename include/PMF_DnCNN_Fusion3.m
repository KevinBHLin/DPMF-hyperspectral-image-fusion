...
function [U,V, noise_list, sigma_list] = PMF_DnCNN_Fusion3(HSI, MSI, Truth, F, psf, start_pos, sigma_hsi, sigma_msi, lambda_u, lambda_v, r, d, iter_num, inner_iter_num, modelfilepath)
...
if(nargin<15)
    modelfilepath = 'NetworkModel\\NetworkTrainedGrayImage\\result.mat'
end
...
%     P = P - 1/ek * tmp;
    ...
    P = single(P);
    vmin = min(reshape(P,[size(P,1),par.size_msi(1)*par.size_msi(2)]),[],2);
    V = bsxfun(@plus,P,-1*vmin);
    V = V./noise_p_sigma;
%     vmin = mean(reshape(P,[size(P,1),par.size_msi(1)*par.size_msi(2)]),2)-0.5;
%     V = bsxfun(@plus,P,-1*vmin);
%     V = V./noise_p_sigma;
    ...
    if useGPU 
        V = gpuArray(V);
        for ch=1:size(V,1)
            imres = vl_simplenn(net,squeeze(V(ch,:,:)),[],[],'conserveMemory',true,'mode','test');
            V(ch,:,:) = squeeze(V(ch,:,:)) + imres(end).x;
        end
        V = gather(V);
    else
        for ch=1:size(V,1)
    ...
    V = V.*noise_p_sigma;
    SNR = min(10*log10(mean(V(:).^2)/noise_p_sigma^2),SNR);
    V = double(bsxfun(@plus,V,vmin));
%     V(V<0) = 0; V(V>1) = 1;
    V = permute(V,[2,3,1]);
