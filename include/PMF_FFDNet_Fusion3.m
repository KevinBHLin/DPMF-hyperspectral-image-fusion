...
function [U,V] = PMF_FFDNet_Fusion3(HSI, MSI, Truth, F, psf, start_pos, sigma_hsi, sigma_msi)
...
%     else
        [~, W, V, res, W_old, V_old] = UpdateWandVStep(Y1, Y2, H*U, V, W, F, par, net, useGPU, j, W_old, V_old);
%     end
    resB2(j) = min(res);
...
function [ U, W, V, res, W_old, V_old ] = UpdateWandVStep( Y1, Y2, U, V, W, F, par, net, useGPU, Iter_count, W_old, V_old )
...
function [ U, W, V, res, W_old, V_old ] = UpdateWandVStep2( Y1, Y2, U, V, W, F, Blur, start_pos, sigma_hsi, sigma_msi, net, size_hsi, size_msi, useGPU, Iter_count, W_old, V_old )
% Solving eq. (7) with a projected gradient descent method.
